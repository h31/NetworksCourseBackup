...
char* getFirstMessage(struct mySocket s){
    if ((s.recv_len = recvfrom(s.sock, s.buf, BUFLEN, 0, (struct sockaddr *) &s.si_other, &s.slen)) == -1)
    {
        die("recvfrom()");
    }
    if (sendto(s.sock, s.buf, s.recv_len, 0, (struct sockaddr*) &s.si_other, s.slen) == -1)
    {
        die("sendto()");
    }
    return s.buf;
...
int main(void)
{
    ...
    while(1)
    {
    	 ...
    	 if (strcmp(command,"transfer")==0){
    		 puts("transfer");
    		 recvfrom(s.sock, s.buf, BUFLEN, 0, (struct sockaddr *) &s.si_other, &s.slen);
    		 sendto(s.sock, s.buf, BUFLEN, 0, (struct sockaddr*) &s.si_other, s.slen);
    		 printf("%s\n",s.buf);
