...
void create_file(const char *to, char *from, char *msg){
	//int rc;
...
void do_exit(char* user){
	printf("Exit command from user: %s\n", user);
	//exit(1);
}
...
struct userThread
{
	char login[MAXUSERS][100];
	int count;
};

DWORD WINAPI startThread(LPVOID in)
{
...
void start_work(int newsockfd){
	char buffer[256];
	int n;
	n = write(newsockfd, "Please, write ur name ending with '#'\n", 38);
	...
	n = write(newsockfd, "Commands: 1#kitty#Hello# - Send message 'Hello' to user kitty;\n"
